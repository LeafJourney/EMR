/**
 * Leafnerd "FHIR Intelligence" — server analytics layer.
 *
 * Source of truth for the demo payload consumed by the `/leafnerd` SPA. This is
 * a faithful, typed port of the prototype's `window.LN` object
 * (see docs/leafnerd-prototype/data.jsx) — the exact strings, numbers, JSON, and
 * Unicode (· — × ↓ ≥) are preserved.
 *
 * Pure server module: NO "use client". `getLeafnerdData()` optionally overlays a
 * handful of real DB aggregates on top of `DEMO_DATA`, but every DB call is
 * wrapped so any failure falls back to the prototype value. The function must
 * NEVER throw and always returns a complete, full-looking payload.
 */
...
const metrics: Metric[] = [
  {
    ...
    cmp: "vs. last 30d",
    insight:
      "Panel grew by 1,440 after onboarding Northbay Clinic's roster on May 18.",
    prov: "Patient · Coverage",
    spark: [38, 40, 41, 40, 43, 45, 44, 47, 48, 48],
  },
  {
    id: "completeness",
    ...
    cmp: "vs. last 30d",
    insight:
      "Demographics and encounters near-complete; social history remains the weakest domain at 61%.",
    prov: "7 domains scored",
    spark: [86, 87, 88, 88, 89, 90, 90, 91, 92, 92],
  },
  {
    id: "mapping",
    label: "FHIR mapping health",
    value: "87.6",
    unit: "%",
    icon: "git",
    tone: "amber",
    ...
    cmp: "vs. last 30d",
    insight:
      "New Northbay feed introduced 312 unmapped MedicationRequest codes pending review.",
    prov: "R4 · US Core 6.1",
    spark: [91, 90, 91, 89, 90, 88, 89, 88, 87, 87],
  },
  {
    id: "caregaps",
    ...
    good: true,
    insight:
      "HbA1c overdue accounts for 38% of open gaps; outreach closed 184 this week.",
    prov: "Quality measures · 9",
    spark: [31, 30, 30, 29, 30, 29, 28, 29, 28, 28],
  },
  {
    id: "risk",
    ...
    cmp: "new this week",
    insight:
      "Rising-risk diabetes sub-cohort drove the increase; 42 patients newly crossed threshold.",
    prov: "HCC + utilization model",
    spark: [112, 114, 113, 116, 115, 118, 117, 119, 120, 121],
  },
];
...
const anomalies: Anomaly[] = [
  {
    id: "a1",
    sev: "high",
    title: "Lab Observation volume dropped 41% from Riverside Lab",
    when: "2h ago",
    detail:
  ...
  },
  {
    id: "a2",
    sev: "med",
    title: "312 MedicationRequest codes unmapped after Northbay onboarding",
    when: "6h ago",
  ...
  },
  {
    id: "a3",
    sev: "med",
    title: "Encounter discharge timestamps missing for 4.2% of records",
    when: "yesterday",
    detail:
  ...
  },
  {
    id: "a4",
    sev: "low",
    title: "Duplicate Patient identities flagged by match engine",
    when: "yesterday",
    detail:
...
const opportunities: Opportunity[] = [
  {
    id: "o1",
    title: "Close 184 HbA1c care gaps before quarter end",
    impact: "+0.6 pts on HEDIS CDC",
    effort: "Low",
    value: 92,
  },
  {
    id: "o2",
    title: "Re-map Northbay medication vocabulary",
    impact: "Restores 312 records to mapped",
    effort: "Medium",
    value: 78,
  },
  {
    id: "o3",
    title: "Outreach to 42 newly rising-risk diabetics",
    impact: "Avoid ~$310K ED utilization",
    effort: "Medium",
    value: 74,
  },
  {
    id: "o4",
    title: "Resolve 58 duplicate patient identities",
    impact: "Improves panel accuracy",
    effort: "Low",
    value: 61,
  },
];
...
const freshness: FreshnessBucket[] = [
  62, 70, 74, 80, 78, 84, 90, 96, 99, 97, 94, 92, 88, 90, 86, 40, 44, 70, 82,
  88, 91, 93, 95, 90,
].map((v, i) => ({
  h: i,
  v,
  state: v < 50 ? "gap" : v < 75 ? "stale" : "ok",
}));
...
const domains: DomainCompleteness[] = [
  ...
  { name: "Encounters", pct: 96 },
  { name: "Conditions", pct: 94 },
  { name: "Medications", pct: 88 },
  { name: "Lab results", pct: 90 },
  { name: "Vitals", pct: 85 },
...
const fhirResources: FhirResource[] = [
  {
    ...
    json: {
      ...
      performer: [
        { reference: "Organization/riverside-lab", display: "Riverside Lab" },
      ],
    },
    related: [
      { t: "Patient", l: "Marcus Delgado" },
      { t: "Encounter", l: "Office visit · 05-28" },
      { t: "DiagnosticReport", l: "Comprehensive metabolic" },
    ],
    provenance: [
      {
        t: "Recorded at source",
        m: "Riverside Lab · HL7v2 ORU · 2026-05-28 09:16",
      },
  ...
  },
  {
    ...
    related: [
      { t: "Patient", l: "Andre Boucher" },
      { t: "Encounter", l: "Telehealth · 05-30" },
    ],
    provenance: [
      { t: "Recorded at source", m: "Northbay EHR · 2026-05-30" },
      { t: "Ingested", m: "2026-05-30 14:02" },
      { t: "Mapped to FHIR R4", m: "confidence 0.96" },
      { t: "Validated", m: "1 warning · missing component code" },
    ],
  },
  {
    id: "cond-1",
    ...
    related: [
      { t: "Patient", l: "Marcus Delgado" },
      { t: "MedicationRequest", l: "Metformin 1000mg" },
    ],
    provenance: [
      { t: "Recorded at source", m: "Northbay EHR · problem list" },
      { t: "Ingested", m: "2024-03-11" },
      { t: "Mapped to FHIR R4", m: "SNOMED 44054006 · 0.99" },
      { t: "Validated", m: "0 errors" },
    ],
  },
  {
    id: "med-1",
    ...
    related: [
      { t: "Patient", l: "Marcus Delgado" },
      { t: "Condition", l: "Type 2 diabetes" },
    ],
    provenance: [
      { t: "Recorded at source", m: "Northbay EHR · local code MTF1000" },
      { t: "Ingested", m: "2026-05-12" },
      { t: "Mapping attempted", m: "RxNorm match 0.58 — below threshold" },
      {
        t: "Validation failed",
        m: "1 error · no recognized coding system",
      },
    ],
  },
  {
    ...
    json: {
      ...
      subject: { reference: "Patient/PT-39114" },
      period: { start: "2026-05-22T10:00:00Z", end: "2026-05-22T10:25:00Z" },
    },
    related: [
      { t: "Patient", l: "Yuki Tanaka" },
      { t: "Observation", l: "HbA1c 7.1%" },
    ],
    provenance: [
      { t: "Recorded at source", m: "Cedar Clinic EHR" },
      { t: "Ingested", m: "2026-05-22" },
      { t: "Mapped to FHIR R4", m: "0.94" },
      { t: "Validated", m: "0 errors" },
    ],
  },
  {
    ...
    provenance: [
      {
        t: "Identity resolved",
        m: "Match engine · 2 sources merged · 0.99",
      },
      { t: "Ingested", m: "2024-01-02" },
      { t: "Mapped to FHIR R4", m: "0.99" },
      { t: "Validated", m: "0 errors" },
    ],
  },
];
...
// --------------------------------------------------------------------------
const fhirCounts: Record<string, number> = {
  Patient: 48210,
  Encounter: 214880,
  Observation: 1840221,
  Condition: 98442,
  MedicationRequest: 142300,
  DiagnosticReport: 64110,
  Coverage: 51200,
};
...
/**
 * The complete, faithful port of `window.LN`. This is the deterministic
 * fallback every surface renders from when no real data is available.
 */
export const DEMO_DATA: LeafnerdData = {
  metrics,
  anomalies,
  opportunities,
  freshness,
...
// NOTE: getLeafnerdData() — the server-only DB overlay — lives in ./server-data.ts
// so THIS module stays pure and client-safe (the SPA's client surfaces import
// DEMO_DATA from here). Do not add prisma/server imports to this file.
