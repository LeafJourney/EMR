...
test.describe("LeafNerd hardening — /leafnerd SPA", () => {
  ...
  test("Cohort Simulator runs a Monte Carlo projection", async ({ page }) => {
    ...
    // Configure the run; assert the control reflects our choice (proves the
    // select is wired) before kicking off the projection.
    const regimen = page.locator("#regimen-select");
    await regimen.selectOption("cbd");
    await expect(regimen).toHaveValue("cbd");
    ...
    // Results render after the staged compute (≈3s of timed steps). These card
    // headings are the stable contract of a completed Monte Carlo run.
  ...
  test("rail user popover exposes a working sign-out", async ({ page }) => {
    ...
    // The popover is a real menu carrying the user identity + a sign-out item.
    await expect(menu).toHaveAttribute("role", "menu");
    await expect(menu.locator(".dropdown-header .nm")).not.toBeEmpty();
    const signOut = menu.locator(".dropdown-item.logout");
    await expect(signOut).toBeVisible();
    await expect(signOut).toContainText("Sign out");

    // The sign-out control is a real affordance — either the dev-mode logout
    // anchor (no Clerk key) or Clerk's <SignOutButton> (a <button>). The actual
    // redirect is owned by the auth layer; we assert the wiring here.
    const wiring = await signOut.evaluate((el) => ({
      tag: el.tagName,
      href: el instanceof HTMLAnchorElement ? el.getAttribute("href") : null,
    }));
    if (wiring.tag === "A") {
      expect(wiring.href).toMatch(/\/api\/dev\/logout|\/sign-in/);
    } else {
      expect(wiring.tag).toBe("BUTTON"); // Clerk <SignOutButton>
    }

    // Escape dismisses the popover without navigating.
    await page.keyboard.press("Escape");
    await expect(menu).toBeHidden();
    await expect(page).toHaveURL(/\/leafnerd/);

    // An outside click also dismisses it (pointer-down-away handler).
    await trigger.click();
    await expect(menu).toBeVisible();
    await page.locator(".page-title").click();
    await expect(menu).toBeHidden();

    // Dev-mode anchor path: clicking sign-out navigates to /sign-in. (The Clerk
    // path needs a live browser session to redirect, which this env lacks, so we
    // only assert the navigation when the affordance is the dev logout link.)
    if (wiring.tag === "A") {
      await trigger.click();
      await expect(menu).toBeVisible();
