...
- **Statement Print + Financial Event Log section collapse.** Each statement tile now has an always-visible **Print** icon (opens the existing branded printable invoice route `invoice/[statementId]`), restructured to sit beside the expand toggle (not nested). The Financial Event Log is now **collapsible at the section level** (`EventLogSection`), not just paginated within.
- **Statement email/text delivery (live-verified).** Each tile has **Email** and **Text** icons (shown only when the patient has that contact on file) that route through the single `deliverMessage` choke-point and surface the **truthful** outcome inline ("Emailed ✓" / "Logged (no provider)" / "Send failed"). A per-patient "Billing & statements" `MessageThread` is found-or-created; the statement is marked sent only when delivery isn't a hard failure. PHI-safe by construction: the message carries only the statement number + a bare-portal link (`buildStatementNotice` in `src/lib/messaging/statement-notice.ts`, unit-tested incl. a no-`$`/no-clinical-terms assertion) — never the balance. Live verify confirmed both branches: SMS→recorded (mock) marked the statement `sms`/fresh `sentAt`; email→failed (dev Resend is sandbox-only — see ops note) correctly did **not** mark it sent. *(New: `statement-notice.ts` + `.test.ts`; `sendStatementAction`.)*
  - **Ops note:** the dev Resend key is in test mode (can only send to the account owner's address); real patient email needs a verified domain + `from` on that domain. SMS needs a Twilio adapter configured. The pipeline is correct; only provider config is missing in dev.
- **Year-end tax summary (live-verified).** The directive's "1099/W9" wording was a mis-spec — the real artifact is a patient **annual out-of-pocket payment summary** (IRS Pub 502), which already existed at `/portal/billing/tax-summary`. Extracted its computation into a shared, unit-tested `src/lib/domain/tax-summary.ts` (`summarizeTaxData` pure + `getPatientTaxSummary`); refactored the portal page onto it **and fixed its broken top "Print summary" button** (`onClick={undefined}` → working client print). Added a **clinician-side** route `clinic/patients/[id]/billing/tax-summary` (same shared computation: hero totals, quarterly breakdown, per-visit service detail, Pub 502 + cannabis disclaimer, year toggle, Print/save-PDF) reached from a **"Generate tax summary"** button at the bottom of the billing tab. **Notify patient** (Email/Text) routes through `deliverMessage` (records to the billing thread = "saveable to Correspondence") with a PHI-safe `buildTaxSummaryNotice` (tested: no `$`/clinical terms). Live verify: 2026 populated (hero total = sum of quarters), 2025 empty-state, year toggle, Text-notify → "Logged (no provider)". *(New: `tax-summary.ts`+test, clinician `tax-summary/` route + `tax-summary-actions.tsx`, `sendTaxSummaryNotice`, `buildTaxSummaryNotice`.)*
...
**Still open on billing (own pass):** payment-method completions (ACH routing/account fields, Bitcoin wallet, Print/Save invoice, cash/check → Correspondence — PCI-sensitive), Insurance **Directory** + click-to-call (telephony), "Cindy suggests" reimbursement-probability panel (deterministic v1, fits the claim-detail popup).
